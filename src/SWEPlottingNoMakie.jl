...
function plotSurf(w, B, dx, dy, nx, ny; show_ground=true, 
    ...
    # A simple cross section along x with y in the middle of the domain for the 
    ...
    #println(gc, (nx, ny))
    x = dx/2:dx:(nx-0.5)*dx
    y = Integer(ceil(ny/2) + gcy)
    if km
        x = x./1000
    end

    fig = Plots.plot(x, w[gc[1]+1:nx+gc[1], y], color=:blue, title=plot_title)
    if show_ground
        Plots.plot!(x, B[gc[1]+1:nx+gc[1], y], color=:red)
    end
