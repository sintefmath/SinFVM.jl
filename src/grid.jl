...
function CartesianGrid(nx; gc = 1, boundary = PeriodicBC(), extent = [0.0 1.0])
    ...
    return CartesianGrid(
        ...
        boundary,
        SVector{2,Float64}(extent[1], extent[2]),
...
function CartesianGrid(nx, ny; gc = 1, boundary = PeriodicBC(), extent = [0.0 1.0; 0.0 1.0])
    ...
    return CartesianGrid(
        SVector{2,Int64}([gc, gc]),
        SVector{2,Int64}([nx + 2 * gc, ny + 2 * gc]),
        boundary,
        SVector{4,Float64}(extent[1, 1], extent[1, 2], extent[2, 1], extent[2, 2]),
        Δ,
    )
end
...
function cell_centers(grid::CartesianGrid{1}; interior = true)
    ...
    xinterface = collect(
        LinRange(
            grid.extent[1],
            grid.extent[2],
            grid.totalcells[1] - 2 * grid.ghostcells[1] + 1,
        ),
    )
...
compute_dx(grid::CartesianGrid{dimension}, direction = XDIR) where {dimension} =
    grid.Δ[direction]


function for_each_inner_cell(f, g::CartesianGrid{1}, include_ghostcells = 0)
    for i =
        (g.ghostcells[1]-include_ghostcells+1):(g.totalcells[1]-g.ghostcells[1]+include_ghostcells)
        f(i - 1, i, i + 1)
    end
end
...
function left_cell(
    ...
    I::Int64,
    direction,
    ghostcells = g.ghostcells[direction],
)
    return I + ghostcells - 1
end

function middle_cell(
    g::CartesianGrid{1},
    I::Int64,
    direction,
    ghostcells = g.ghostcells[direction],
