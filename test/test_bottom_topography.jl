...
@test SinSWE.B_face_minus(B_const_default, 45, 12) == 0
@test SinSWE.B_face_plus(B_const_default, 45) == 0

B_const = SinSWE.ConstantBottomTopography(3.14)
@test B_const.B == 3.14
@test SinSWE.B_cell(B_const, 45) == 3.14
@test SinSWE.B_face_minus(B_const, 45, 12) == 3.14
@test SinSWE.B_face_plus(B_const, 45) == 3.14
...
for backend in SinSWE.get_available_backends()
    ...
    CUDA.@allowscalar @test SinSWE.B_face_plus(B1, 4+2) ≈ 4 atol=10^-10
    CUDA.@allowscalar @test SinSWE.B_face_plus(B1, 7+2) ≈ 7 atol=10^-10
    CUDA.@allowscalar @test SinSWE.B_face_minus(B1, 4+2) ≈ 3 atol=10^-10
    CUDA.@allowscalar @test SinSWE.B_face_minus(B1, 7+2) ≈ 6 atol=10^-10
