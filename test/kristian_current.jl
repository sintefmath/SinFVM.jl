...
function cost_function_2D_house(params)
    ...
    if total_water_at_cell > 10^-1
        water_damage_cost = 1000 * (total_water_at_cell - 10^-1)^2
    else
        water_damage_cost = 0
    end
    ...
    open("cost_function.csv", "a") do io
        ...
        dbv = ForwardDiff.value(water_damage_cost)
...
initial_guess = [1.3, 120, 20]  # Initial guess with position as float for optimization
